<template>
     <div class="card border-0 shadow position-relative">
          ...
          <div class="card-body">
               ...
               <h5 class="card-title mt-4 fw-semibold">{{truncatedSensorTitle}}</h5>
               <p class="card-text mb-3">{{ truncatedText }}</p>
               <div class="d-flex justify-content-between align-items-center">
                    <a href="#" class=card-link> Read More </a>
                    <p style="background-color: var(--primary-color);" class="py-1 px-2 text-white badge mb-0">{{sensor.uploadDate}}</p>
...
export default {
     ...
     computed : {
          ratingBadgeClass(){
               if(this.sensor.rating > 7 ) {
                    return 'bg-success'
               } else if (this.sensor.rating > 4 ){
                    return 'bg-warning'
               } else {
                    return 'bg-danger'
               }
          ...
          truncatedText(){
               if(this.sensor.description.length >50){
                    return this.sensor.description.slice(0,50) + '...';
               }
               return this.sensor.description;
          },
          truncatedSensorTitle(){
               if(this.sensor.title.length > 18){
                    return this.sensor.title.slice(0,18) + '...';
               }
               return this.sensor.title;
          }
     }
}
</script>
